    ...
    public APIResponse<?> updateStatus(
        ...
        if (validator.valid()) {
            ...
            if (payment.isPresent()) {
                ...
                if (requestBody.get("proof_of_payment") != null) {
                    p.proof_of_payment = this.fileUtil.storeBase64ToTemp(String.valueOf(requestBody.get("proof_of_payment"))).path;
                }

                p = this.repository.save(p);

                /*Create production data*/
                Optional<Production> getProduction = productionRepository.findByPaymentId(p.id.intValue());
                Production production = new Production();

                if (getProduction.isPresent()) {
                    production = getProduction.get();
                }
                production.payment_id = p.id.intValue();
